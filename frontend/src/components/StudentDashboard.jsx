    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`mt-1 ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>Track your job search progress</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className={`rounded-2xl p-6 transition-all hover:scale-105 ${isDark ? 'bg-slate-800/50 border border-slate-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>Jobs Applied</p>
              <p className={`text-3xl font-bold mt-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>{stats.jobsApplied || 0}</p>
              <p className={`text-xs mt-1 flex items-center ${
                (stats.jobsAppliedChange || 0) >= 0
                  ? (isDark ? 'text-green-400' : 'text-green-600')
                  : (isDark ? 'text-red-400' : 'text-red-600')
              }`}>
                <TrendingUp className={`h-3 w-3 mr-1 ${(stats.jobsAppliedChange || 0) < 0 ? 'rotate-180' : ''}`} />
                {(stats.jobsAppliedChange || 0) >= 0 ? `+${stats.jobsAppliedChange || 0}%` : `${stats.jobsAppliedChange || 0}%`} from last month
              </p>
            </div>
        ...
        <div className={`rounded-2xl p-6 transition-all hover:scale-105 ${isDark ? 'bg-slate-800/50 border border-slate-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>Interviews</p>
              ...
              <p className={`text-xs mt-1 flex items-center ${isDark ? 'text-green-400' : 'text-green-600'}`}>
                ...
                {stats.interviewsText || '0 upcoming'}
              </p>
            </div>
        ...
        <div className={`rounded-2xl p-6 transition-all hover:scale-105 ${isDark ? 'bg-slate-800/50 border border-slate-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-3xl font-bold mt-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>{stats.pending || 0}</p>
              <p className={`text-xs mt-1 ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`}>{stats.pendingText || 'Awaiting response'}</p>
            </div>
        ...
        <div className={`rounded-2xl p-6 transition-all hover:scale-105 ${isDark ? 'bg-slate-800/50 border border-slate-700' : 'bg-white border border-gray-200 shadow-lg'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>Profile Score</p>
              <p className={`text-3xl font-bold mt-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>{stats.profileScore || 0}%</p>
              <p className={`text-xs mt-1 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>{stats.profileScoreText || 'Needs update'}</p>
            </div>
        ...
        <div className="space-y-4">
          {stats.recentActivity && stats.recentActivity.length > 0 ? (
            stats.recentActivity.map((activity, index) => {
              ...
              } else if (activity.icon === 'file_text') {
